...
abstract contract FIDNameResolver {
...
contract ReverseRegistrar is Ownable, Controllable {
    FID public fid;
    FIDNameResolver public defaultResolver;

    event ReverseClaimed(address indexed addr, bytes32 indexed node);

    /**
     * @dev Constructor
     * @param fidAddr The address of the FID registry.
     * @param resolverAddr The address of the default reverse resolver.
     */
    constructor(FID fidAddr, FIDNameResolver resolverAddr) {
        fid = fidAddr;
        defaultResolver = resolverAddr;

        // Assign ownership of the reverse record to our deployer
        ReverseRegistrar oldRegistrar = ReverseRegistrar(
            fid.owner(ADDR_REVERSE_NODE)
        );
    ...
    modifier authorised(address addr) {
        require(
            addr == msg.sender ||
                ...
                fid.isApprovedForAll(addr, msg.sender) ||
                ownsContract(addr),
    ...
    /**
     * @dev Transfers ownership of the reverse FID record associated with the
     *      calling account.
     * @param owner The address to set as the owner of the reverse record in FID.
    ...
    /**
     * @dev Transfers ownership of the reverse FID record specified with the
     *      address provided
     * @param addr The reverse record to set
     * @param owner The address to set as the owner of the reverse record in FID.
     * @param resolver The address of the resolver to set; 0 to leave unchanged.
    ...
    /**
     * @dev Sets the `name()` record for the reverse FID record associated with
